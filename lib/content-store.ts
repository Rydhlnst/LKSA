...
async function getFileContent(): Promise<SiteContent> {
  try { return JSON.parse(await fs.readFile(contentPath, "utf8")) as SiteContent; } catch { return applyContentOverrides(defaultContent); }
}

export async function getSiteContent(): Promise<SiteContent> {
  if (db) {
    const rows = await db.select().from(siteSettings).where(eq(siteSettings.id, singletonId)).limit(1);
    if (rows[0]?.data) return rows[0].data as SiteContent;
  }
