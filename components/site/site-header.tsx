...
export function SiteHeader({ settings }: { settings: SiteSettings }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-navy shadow-subtle backdrop-blur">
      <div className="site-container flex h-20 items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-6 desktop:flex" aria-label="Navigasi utama">
          ...
          <div className="relative" onMouseEnter={() => setProfileOpen(true)} onMouseLeave={() => setProfileOpen(false)}>
            ...
            {profileOpen && <div className="absolute left-0 top-full mt-3 w-56 rounded-2xl border border-white/10 bg-navy p-2 shadow-card">{profileLinks.map(([label, href]) => <Link key={href} href={href} className="block rounded-xl px-3 py-2.5 text-sm text-brand-yellow/90 transition hover:bg-white/10 hover:text-white">{label}</Link>)}</div>}
          </div>
        ...
        <button type="button" className="rounded-xl p-2 text-brand-yellow transition hover:bg-white/10 hover:text-white desktop:hidden" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="mobile-navigation">
          <span className="sr-only">Buka menu utama</span>
          {open ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>
      </div>
      {open && <div id="mobile-navigation" className="border-t border-white/10 bg-navy px-4 pb-6 pt-3 shadow-elevated desktop:hidden">
        <Link href="/" onClick={() => setOpen(false)} className="mobile-nav-link text-brand-yellow">Beranda</Link>
        <p className="px-3 pb-1 pt-4 text-[11px] font-bold uppercase tracking-[0.18em] text-brand-yellow/65">Profil Panti</p>
